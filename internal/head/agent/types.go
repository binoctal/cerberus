...
type TestStep struct {
	...
	// http_request: expected status CLASS (2xx|3xx|4xx|5xx|2xx_4xx|any)
	// evaluated after ExpectStatus (exact status wins when both set). "any"
	// passes on any HTTP response — transport errors (status 0) still fail,
	// proving only reachability; "2xx_4xx" gates on "no server error, no
	// routing error" (authed mutations accept legitimate 4xx rejections).
	// An unknown value fails the step with an error.
