...
Batches: a protocol may declare batches — a batch routing type that carries N logical items at an items_path (e.g. session:output-batch holds an array of session:output items). The executor decomposes one batch frame into N per-item frames automatically, so ws_receive of the item type (not the batch type) matches each item in arrival order; a batch-routing-type receive no longer sees the raw batch. Set match_all=true on a ws_receive to collect EVERY item in the burst and check assert against EACH — the receive passes only when every item satisfies every assert, and the first bad item fails it (1-based index named in the error). Use match_all for "every item satisfies P" checks where the item count is unknown at authoring time; without it a receive matches only the first item.
