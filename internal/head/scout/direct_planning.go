...
func (s *Scout) runAIPlanning(ctx context.Context, prompt string, goal string, model *project.ProjectModel) (*agent.TestPlan, map[string]map[string]bool, map[string]map[string]string, map[string]map[string]string, error) {
	...
	if err != nil {
		...
		return fb, map[string]map[string]bool{}, map[string]map[string]string{}, map[string]map[string]string{}, nil
	}
	if len(res.ToolCalls) == 0 {
		...
		return &agent.TestPlan{}, map[string]map[string]bool{}, map[string]map[string]string{}, map[string]map[string]string{}, nil
	}
	...
	plan, covered, coveringCase, httpCovering := assemblePlan(res.ToolCalls, goal, s.resolveBaseURL(), s.config.Services)
