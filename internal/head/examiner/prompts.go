...
- For a 'fail' verdict, also set redispatch_hint to the correctable root cause if any: 'endpoint_drift' (wrong path/method/verb), 'auth' (missing/bad credentials or scheme), or 'shape' (wrong payload/contract). For a WebSocket failure use 'handshake' (a declared/role handshake did not arrive or the awaited message type is wrong), 'ws_shape' (the ws_send message envelope/payload does not match the protocol), or 'ws_match' (a ws_receive matched nothing decisive, or an assert/match_all predicate failed). Use 'none' for pass/skip or any non-correctable failure. Put the diagnostic detail in reasoning, not in a separate field.`
