...
func TestRenderJUnit_AllVerdicts(t *testing.T) {
	...
	_, err = s.CreateVerdict(ctx, sess.ID, trace1, "GET /api/health", "pass", 0.99, "judge", "healthy", nil, store.FailureReasonNone)
	require.NoError(t, err)
	_, err = s.CreateVerdict(ctx, sess.ID, trace2, "POST /api/users", "fail", 0.85, "judge", "duplicate email", nil, store.FailureReasonAssertionFailed)
	require.NoError(t, err)
	_, err = s.CreateVerdict(ctx, sess.ID, trace3, "GET /api/skip", "skip", 0.0, "judge", "not applicable", nil, store.FailureReasonNone)
	require.NoError(t, err)
	_, err = s.CreateVerdict(ctx, sess.ID, trace4, "GET /api/unknown", "uncertain", 0.5, "judge", "unexpected status", nil, store.FailureReasonNone)
	require.NoError(t, err)
...
func TestRenderJUnit_PassOnly(t *testing.T) {
	...
	require.NoError(t, s.FinishTrace(ctx, trace1, "pass"))
	_, err = s.CreateVerdict(ctx, sess.ID, trace1, "GET /ok", "pass", 1.0, "judge", "all good", nil, store.FailureReasonNone)
	require.NoError(t, err)
...
func TestRenderJUnit_WithEvidence(t *testing.T) {
	...
	_, err = s.CreateVerdict(ctx, sess.ID, trace1, "POST /api/login", "fail", 0.7, "judge", "auth failed", nil, store.FailureReasonAssertionFailed)
	require.NoError(t, err)
...
func TestBuildReport_WithEvidence(t *testing.T) {
	...
	require.NoError(t, s.FinishTrace(ctx, trace1, "pass"))
	_, err = s.CreateVerdict(ctx, sess.ID, trace1, "GET /api/items", "pass", 0.95, "judge", "ok", nil, store.FailureReasonNone)
	require.NoError(t, err)
