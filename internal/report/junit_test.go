...
func TestRenderJUnit_AllVerdicts(t *testing.T) {
	...
	_, err = s.CreateVerdict(ctx, sess.ID, trace1, "GET /api/health", "pass", 0.99, "judge", "healthy", nil)
	require.NoError(t, err)
	_, err = s.CreateVerdict(ctx, sess.ID, trace2, "POST /api/users", "fail", 0.85, "judge", "duplicate email", nil)
	require.NoError(t, err)
	_, err = s.CreateVerdict(ctx, sess.ID, trace3, "GET /api/skip", "skip", 0.0, "judge", "not applicable", nil)
	require.NoError(t, err)
	_, err = s.CreateVerdict(ctx, sess.ID, trace4, "GET /api/unknown", "uncertain", 0.5, "judge", "unexpected status", nil)
	require.NoError(t, err)
