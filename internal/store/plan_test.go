...
func TestGetCompletedTargets(t *testing.T) {
	...
	_, err = s.CreateVerdict(ctx, sess.ID, trace1, "GET /a", "pass", 0.9, "judge", "ok", nil, FailureReasonNone)
	require.NoError(t, err)

	trace2, _ := s.CreateTrace(ctx, sess.ID, "http", "GET /b")
	require.NoError(t, s.FinishTrace(ctx, trace2, "fail"))
	_, err = s.CreateVerdict(ctx, sess.ID, trace2, "GET /b", "fail", 0.8, "judge", "err", nil, FailureReasonAssertionFailed)
	require.NoError(t, err)
