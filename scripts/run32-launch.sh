...
echo "[run32] $(date -Is) starting wrangler :8989" >> "$LOG_DIR/run32-launcher.log"
cd "$OA_API" || exit 1
eval "$(fnm env)" && fnm use 22 >/dev/null 2>&1
(nohup npm run dev > "$LOG_DIR/run32-wrangler.log" 2>&1 &)
...
for i in $(seq 1 60); do
  ...
  if [ "$code" != "000" ] && [ -n "$code" ]; then
    echo "[run32] $(date -Is) wrangler up (health $code)" >> "$LOG_DIR/run32-launcher.log"
    break
  fi
  sleep 2
done
...
source ../../scripts/dogfood-realtime-e2e-env.sh >> "$LOG_DIR/run32-launcher.log" 2>&1
echo "[run32] $(date -Is) launching cerberus run" >> "$LOG_DIR/run32-launcher.log"
../../build/cerberus run > "$LOG_DIR/run32-stdout.log" 2>&1
echo "[run32] $(date -Is) cerberus run exited rc=$?" >> "$LOG_DIR/run32-launcher.log"
