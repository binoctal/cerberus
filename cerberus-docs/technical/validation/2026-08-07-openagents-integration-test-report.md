...
A `>0` coverage fraction requires a run that actually exchanges a WS message edge (a `ws_send` of type T from one role + a matched `ws_receive` of T by another). Follow-up runs (3 total) traced why this does NOT happen against the current `ws-realtime` dogfood, and the root cause is structural, not auth: `internal/head/scout/ws_cases.go:210` skips relay-case generation unless `len(svc.Protocol.Roles) >= 2`, but `dogfood/ws-realtime/.cerberus/protocols/open-agents.yaml` declares only the `web` role (1 role). With no bridge role, cerberus generates only `web-connect` cases, which exchange no message types, so `exercisedEdges` is correctly empty and path coverage is honestly 0. (Secondary friction: those connect cases also hit `endpoint_drift` — the protocol declares no `/ws/<userId>` path template or provisioning hook, so they dial the bare host — and auth drifts to `web-token`/`web-actor` instead of the dev `demo_token`; all pre-existing dogfood/protocol gaps, unrelated to this change.) Demonstrating a live `>0` therefore requires extending the dogfood protocol (declare `bridge` + a path/provisioning template) — separate work. The `>0` math itself is unit-proven (`internal/session` `TestExercisedEdges`: 1/2 edges → `Pct 0.5`; `TestPathCoverage_OutOfBandDoesNotInflate` proves out-of-band edges cannot inflate it). First run stats: 3 pass / 1 fail / 1 skip, ~16K tokens, 1m25s.
